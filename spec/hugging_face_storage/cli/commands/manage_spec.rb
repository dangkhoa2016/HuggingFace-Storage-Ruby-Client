...
RSpec.describe HuggingFaceStorage::CliCommands::Manage do
  ...
  describe "#list" do
    let(:dir_info) do
      HuggingFaceStorage::EntryInfo.new(type: "directory", path: "models", mtime: "2026-01-15")
    end

    let(:file_info) do
      HuggingFaceStorage::EntryInfo.new(type: "file", path: "file.txt", size: 100, xet_hash: "abcdef123456", mtime: "2026-01-01")
    end
    ...
    it "delegates to FileManager#list_entries" do
      commands.list(bucket, "src")
      expect(files).to have_received(:list_entries).with(hash_including(prefix: "src"))
    end

    it "outputs formatted rows with [DIR] prefix for directories" do
      ...
      expect(HuggingFaceStorage::CLIFormatter).to have_received(:format_output).with(
        array_including(
          array_including("file.txt", 100, "abcdef123456", "2026-01-01"),
          array_including("[DIR] models", nil, nil, "2026-01-15")
        ),
        nil,
    ...
    context "when no entries found" do
      ...
      it "says 'No entries found'" do
        expect(commands).to receive(:say).with("No entries found")
        commands.list(bucket)
      end
    end
    ...
    context "with json format" do
      ...
      it "outputs formatted rows with [DIR] prefix for directories in json" do
        allow(HuggingFaceStorage::CLIFormatter).to receive(:format_output)
        commands.list(bucket)
        expect(HuggingFaceStorage::CLIFormatter).to have_received(:format_output).with(
          anything,
    ...
    context "when an error occurs" do
      before do
        allow(files).to receive(:list_entries).and_raise(HuggingFaceStorage::Error, "list failed")
      end
