...
module HuggingFaceStorage
  ...
  class XetStreamProcessor
    ...
    def stream_upload(remote_path, cas_url:, token:, cancel_token: nil, &block)
      ...
      state = init_stream_state(rep_builder = @serializer.stream_representation_builder)

      yield make_write_chunk(state, rep_builder, cas_url, token, cancel_token)

      @cdc_chunker.finalize { |chunk| buffer_streaming_chunk(chunk, state) }
      flush_streaming_batch(rep_builder, cas_url, token, cancel_token: cancel_token)
      cancel_token&.raise_if_cancelled!
      flush_current_xorb(cas_url, token, rep_builder, cancel_token: cancel_token)
      finalize_and_upload(remote_path, state[:sha256_ctx], state[:total_bytes], cas_url, token, rep_builder,
                          cancel_token: cancel_token)
    end

    private

    def init_stream_state(_rep_builder)
      @all_chunk_pairs = []
      @uploaded_xorbs = []
      @pending_xorb_chunks = []
      @pending_xorb_size = 0
      { sha256_ctx: Digest::SHA256.new, total_bytes: 0 }
    end

    def make_write_chunk(state, rep_builder, cas_url, token, cancel_token)
      lambda do |http_chunk|
        http_chunk = http_chunk.b
        state[:sha256_ctx] << http_chunk
        state[:total_bytes] += http_chunk.bytesize
        @pending_streaming_chunks ||= []
        @cdc_chunker.feed(http_chunk) do |chunk|
          buffer_streaming_chunk(chunk, state)
          if @pending_streaming_chunks.length >= STREAMING_BATCH_THRESHOLD
            flush_streaming_batch(rep_builder, cas_url, token,
                                  cancel_token: cancel_token)
          end
        end
      end
    ...
    def buffer_streaming_chunk(chunk, _state)
      @pending_streaming_chunks ||= []
      @pending_streaming_chunks << chunk
    end
    ...
    def flush_streaming_batch(rep_builder, cas_url, token, cancel_token: nil)
      return if @pending_streaming_chunks.nil? || @pending_streaming_chunks.empty?

      chunks = @pending_streaming_chunks
      @pending_streaming_chunks = []

      hashes = @hasher.batch_blake3_keyed(XetHasher::DATA_KEY, chunks, cancel_token: cancel_token)
      chunks.each_with_index do |chunk_bytes, i|
        entry = { data: chunk_bytes, hash: hashes[i], length: chunk_bytes.bytesize }
        chunk_total = XetHasher::CHUNK_HEADER_SIZE + chunk_bytes.bytesize
        if @pending_xorb_size + chunk_total > XetHasher::XORB_MAX_SIZE || @pending_xorb_chunks.length >= XetHasher::XORB_MAX_CHUNKS
          flush_current_xorb(cas_url, token, rep_builder, cancel_token: cancel_token)
        end
        rep_builder.start_xorb if @pending_xorb_chunks.empty?
        rep_builder.add_chunk(hashes[i], chunk_bytes.bytesize)
        @pending_xorb_chunks << entry
        @pending_xorb_size += chunk_total
      end
    end
