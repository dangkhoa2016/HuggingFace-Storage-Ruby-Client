...
module HuggingFaceStorage
  ...
  module BucketQuery
    ...
    def fetch_file_info(api, bucket_id, path)
      raw = query_paths(api, bucket_id, [path])
      raw ||= []
      raise NotFoundError, "File not found: #{path}" if raw.empty?
    ...
    def ensure_file!(api, bucket_id, path)
      results = query_paths(api, bucket_id, [path])
      results ||= []
      raise NotFoundError, "File not found: #{path}" if results.empty?

      entry = results.fetch(0)
