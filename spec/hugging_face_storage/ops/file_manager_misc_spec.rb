...
RSpec.describe HuggingFaceStorage::FileManager do
  ...
  describe "debug mode" do
    ...
    it "preserves backtrace when debug_mode=true" do
      ...
      dm_cp = HuggingFaceStorage::CopyPipeline.new(
        api_client: api, xet_uploader: uploader, bucket_id: bucket_id, logger: logger, config: dm_cfg
      )
      dm_sbc = HuggingFaceStorage::SameBucketCopyService.new(
        api_client: api, bucket_id: bucket_id, logger: logger, config: dm_cfg
      )
      dm_si = HuggingFaceStorage::SourceIterator.new(
        api: api, bucket_id: bucket_id, logger: logger, debug_mode: true
      )
      dm_crc = HuggingFaceStorage::CrossRepoCopyService.new(
        api_client: api, file_manager: nil, copy_pipeline: dm_cp, bucket_id: bucket_id,
        source_iterator: dm_si, logger: logger
      )
      dm_cs = HuggingFaceStorage::FileCopyService.new(
        same_bucket: dm_sbc, cross_repo: dm_crc, copy_pipeline: dm_cp, logger: logger, config: dm_cfg
      )
  ...
  describe "#ensure_files_exist!" do
    it "delegates to BucketQuery.ensure_files!" do
      paths = ["a.txt", "b.txt"]
      expect(HuggingFaceStorage::BucketQuery).to receive(:ensure_files!).with(api, bucket_id, paths)
      fm.send(:ensure_files_exist!, paths)
    end
  end
  ...
  describe "custom Configuration" do
    it "respects custom delete_batch_size" do
      ...
      custom_delete_service = HuggingFaceStorage::FileDeleteService.new(
        api_client: api, bucket_id: bucket_id, config: config, logger: logger
      )
      custom_fm = described_class.new(
        api_client: api, xet_uploader: uploader, xet_downloader: downloader,
        bucket_id: bucket_id, logger: logger, config: config,
        upload_service: upload_service, delete_service: custom_delete_service,
