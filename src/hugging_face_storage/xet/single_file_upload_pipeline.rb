...
module HuggingFaceStorage
  ...
  class SingleFileUploadPipeline
    ...
    def stream_and_upload_data(bucket_id, data, cas_url, remote_path, on_progress, cancel_token, &block)
      chunk_ranges, xorb_hash, file_hash, sha256, representation, chunk_hashes, chunk_lengths =
        compute_single_file_metadata(data, cancel_token, &block)

      cancel_token&.raise_if_cancelled!
      serializer = XetSerializer.new
      xorb_serialized = serializer.serialize_xorb_from_ranges(data, chunk_ranges)
    ...
    def compute_single_file_metadata(data, cancel_token, &block)
      chunk_ranges, _source_data, chunk_hashes, chunk_lengths, sha256 =
        yield(data, cancel_token: cancel_token)
