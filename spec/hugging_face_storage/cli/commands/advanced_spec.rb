...
RSpec.describe HuggingFaceStorage::CliCommands::Advanced do
  ...
  describe "#edit" do
    ...
    context "when --edits JSON is invalid" do
      before do
        commands.options = { edits: "not valid json{" }
      end

      it "calls error with formatted JSON parse error message" do
        allow(HuggingFaceStorage::CLIFormatter).to receive(:format_error)
          .with(/Invalid --edits JSON/)
          .and_return("ERROR: Invalid JSON")
        expect(commands).to receive(:error).with("ERROR: Invalid JSON")
        commands.edit(bucket, "config.json")
      end
    end
  end
