...
module HuggingFaceStorage
  class FileManager
    ...
    class Lister
      ...
      def list_entries(prefix: nil, recursive: false)
        @logger.info("Listing entries: prefix=#{prefix || 'root'} recursive=#{recursive}")
        entries = list_entries_with_pagination(prefix, recursive)

        result = entries.map do |e|
          entry_type = e[ResponseFields::TYPE]
          EntryInfo.new(
            type: entry_type,
            path: e[ResponseFields::PATH],
            size: e[ResponseFields::SIZE],
            xet_hash: e[ResponseFields::XET_HASH],
            mtime: e[ResponseFields::MTIME] || e["uploadedAt"]
          )
        end
        @logger.info("Found #{result.size} entry(ies)")
        result
      end
