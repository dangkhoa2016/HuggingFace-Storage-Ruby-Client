...
RSpec.describe HuggingFaceStorage::XetStreamProcessor do
  ...
  describe "#stream_upload" do
    ...
    it "uses batch_blake3_keyed when enough chunks are buffered in streaming" do
      allow(hasher).to receive(:batch_blake3_keyed).and_call_original
      ...
      # Feed enough data to create multiple chunks
      processor.stream_upload("remote/batch.bin", cas_url: cas_url, token: token) do |write_chunk|
        write_chunk.call("a" * 200_000)
      end

      expect(hasher).to have_received(:batch_blake3_keyed).at_least(:once)
    end
  end
end
