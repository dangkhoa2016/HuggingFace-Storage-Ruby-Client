...
RSpec.describe "NoopCloseIO" do
  let(:noop_io) { HuggingFaceStorage.const_get(:NoopCloseIO).new(StringIO.new) }

  it "returns false for closed?" do
    expect(noop_io.closed?).to be(false)
  end

  it "does not close the underlying IO" do
    noop_io.close
    expect(noop_io.closed?).to be(false)
  end
end
