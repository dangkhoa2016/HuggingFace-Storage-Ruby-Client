...
RSpec.describe HuggingFaceStorage::BatchFileUploadPipeline do
  ...
  describe "#process_file_chunks" do
    ...
    let(:source_data) { chunk_data }
    let(:chunk_ranges) { [[0, 100]] }
    let(:chunk_length) { 100 }

    it "accumulates chunks into pending state" do
      chunk_count = pipeline.process_file_chunks(
        bucket_id, cas_url, state, chunk_ranges, source_data, [chunk_hash], [chunk_length], cancel_token
      )
      ...
      # Verify tuple format is used
      chunk = state.pending_chunks[0]
      expect(chunk).to be_a(Array)
      expect(chunk[0]).to eq(source_data)  # source
      expect(chunk[1]).to eq(0)           # offset
      expect(chunk[2]).to eq(100)         # length
      expect(chunk[3]).to eq(chunk_hash)  # hash
    end

    it "flushes pending chunks when xorb max chunks is exceeded" do
      many_ranges = (xorb_max_chunks + 1).times.map { |i| [i * 100, (i + 1) * 100] }
      many_source = "x" * ((xorb_max_chunks + 1) * 100)
      ...
      pipeline.process_file_chunks(
        bucket_id, cas_url, state, many_ranges, many_source, many_hashes, many_lengths, cancel_token
      )
  ...
  describe "#flush_pending_xorb" do
    before do
      # Use tuple format: [source, offset, length, hash]
      state.pending_chunks << ["test", 0, 4, chunk_hash]
  ...
  describe "#process_single_file_entry" do
    ...
    it "processes a file entry and adds file meta" do
      cdc_result = [[[0, 100]], "x" * 100, [chunk_hash], [100], [{ hash: chunk_hash, length: 100 }], nil]
      pipeline.process_single_file_entry(bucket_id, cas_url, state, entry, 0, cancel_token, precomputed: cdc_result)
    ...
    it "raises CancelledError when token is cancelled" do
      ...
      expect do
        pipeline.process_single_file_entry(bucket_id, cas_url, state, entry, 0, cancel_token,
                                           precomputed: [[[0, 100]], "x" * 100, [chunk_hash], [100], [], nil])
...
cancel_token: cancel_token) do |e, cancel_token:|
        [[[0, e[:size]]], "x" * e[:size], [chunk_hash], [e[:size]], [{ hash: chunk_hash, length: e[:size] }], nil]
      end
