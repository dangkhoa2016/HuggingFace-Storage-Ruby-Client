...
RSpec.describe HuggingFaceStorage::TokenRetryable do
  ...
  context "when label contains underscores" do
    let(:flexible_token_manager) do
      Class.new do
        define_method(:fetch_write_token) do |_bid|
          { token: "fresh" }
        end

        define_method(:invalidate_write_token) { |_bid| nil }
      end.new
    end

    before do
      instance.token_manager = flexible_token_manager
    end

    it "formats label name with spaces and capitalization in log message" do
      log_message = nil
      ...
      first_attempt = true
      instance.send(:with_token_retry, bucket_id, label: :write) do |token|
        if first_attempt
          first_attempt = false
          raise HuggingFaceStorage::ApiError.new(message: "unauthorized", status: 401)
        end
      ...
      expect(log_message).to eq("Write token invalidated (HTTP 401), refreshing and retrying")
    end
  end

  context "when 401 error occurs" do
    before do
      call_count = 0
      allow(token_manager).to receive(:fetch_read_token).with(bucket_id) do
        call_count += 1
    ...
    it "logs formatted label name in warn message" do
      log_message = nil
      allow(logger).to receive(:warn) do |&block|
        log_message = block.call
      end
      ...
      first_attempt = true
      instance.send(:with_token_retry, bucket_id, label: :read) do |token|
        if first_attempt
          first_attempt = false
          raise HuggingFaceStorage::ApiError.new(message: "unauthorized", status: 401)
        end
      ...
      expect(log_message).to eq("Read token invalidated (HTTP 401), refreshing and retrying")
    end
  end
end
