...
RSpec.describe HuggingFaceStorage::CLI do
  ...
  describe "list" do
    it "lists files in table format" do
      file_info = HuggingFaceStorage::FileInfo.new(path: "file.txt", size: 100, xet_hash: "abcdef123456",
mtime: "2026-01-01")
  ...
  describe "list --format" do
    it "outputs JSON when --format json" do
      file_info = HuggingFaceStorage::FileInfo.new(path: "f.txt", size: 100, xet_hash: "abc123def456",
mtime: "2026-01-01")
      allow(files).to receive(:list).and_return([file_info])
    ...
    it "shows message when no files found (table default)" do
      allow(files).to receive(:list).and_return([])
  ...
  describe "list --recursive" do
    it "passes recursive: true when --recursive is given" do
      ...
      expect(files).to have_received(:list).with(hash_including(recursive: true))
    end
    ...
    it "defaults recursive to false" do
      ...
      expect(files).to have_received(:list).with(hash_including(recursive: false))
    end
  end
