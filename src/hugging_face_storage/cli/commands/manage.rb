...
module HuggingFaceStorage
  ...
  module CliCommands
    ...
    module Manage
      ...
      # Lists files and directories in a bucket, optionally filtered by prefix.
      #
      # @param bucket [String] bucket identifier
      ...
      def list(bucket, path = nil)
        client = shared_client(bucket)
        entries = client.files.list_entries(prefix: path, recursive: options[:recursive])
        if entries.empty?
          say "No entries found"
        else
          rows = entries.map do |e|
            display_path = e.directory? ? "[DIR] #{e.path}" : e.path
            [display_path, e.size, e.xet_hash&.slice(0, 12), e.mtime]
          end
