...
RSpec.describe HuggingFaceStorage::SingleFileUploadPipeline do
  ...
  let(:cdc_result) do
    ...
    [[[0, data.bytesize]], data, [chunk_hash], [data.bytesize], sha256]
  end
  ...
  before do
    ...
    allow(HuggingFaceStorage::XetSerializer).to receive_message_chain(:new, :serialize_xorb_from_ranges).and_return(xorb_serialized)
...
cancel_token) do |d, cancel_token:|
  ...
  describe "#compute_single_file_metadata" do
    it "computes metadata from cdc result" do
      ...
      expect(result[0]).to eq([[0, data.bytesize]])
